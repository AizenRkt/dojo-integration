...
class TarifEcolageModel {
    ...
    public function insert($montant, $adult) {
        try {
            $db = Flight::db();
            $stmt = $db->prepare("INSERT INTO tarif_ecolage (montant, adult, date_modif) VALUES (:montant, :adult, NOW())");
    ...
    public function getCurrentTarifAdulte() {
        try {
            $db = Flight::db();
            $stmt = $db->query("SELECT * FROM tarif_ecolage WHERE adult = true ORDER BY date_modif DESC LIMIT 1");
    ...
    public function getCurrentTarifEnfant() {
        try {
            $db = Flight::db();
            $stmt = $db->query("SELECT * FROM tarif_ecolage WHERE adult = false ORDER BY date_modif DESC LIMIT 1");
